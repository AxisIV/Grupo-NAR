...
export default defineComponent({
  setup() {
    ...
    const addOption = (index: number) => {
      const newValue = "Opcion" + questions.value[index].opciones.length + 1;
      questions.value[index].opciones.push(newValue);
    ...
    const updateQuestionType = (index: number, newType: string) => {
      ...
      const newValue = "Opcion" + index;
      questions.value[index].opciones = [newValue]; // Reiniciar opciones si es pregunta de opción múltiple
    };
